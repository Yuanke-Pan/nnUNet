...
def get_moreDA_augmentation(dataloader_train, dataloader_val, dataloader_un, patch_size, params=default_3D_augmentation_params,
                            border_val_seg=-1,
    ...
    if params.get("selected_seg_channels") is not None:
        unlabel_transforms.append(SegChannelSelectionTransform(params.get("selected_seg_channels")))
    ...
    if params.get("do_mirror") or params.get("mirror"):
        unlabel_transforms.append(MirrorTransform(params.get("mirror_axes")))

    if params.get("mask_was_used_for_normalization") is not None:
        mask_was_used_for_normalization = params.get("mask_was_used_for_normalization")
        unlabel_transforms.append(MaskTransform(mask_was_used_for_normalization, mask_idx_in_seg=0, set_outside_to=0))

    unlabel_transforms.append(RemoveLabelTransform(-1, 0))

#    if params.get("move_last_seg_chanel_to_data") is not None and params.get("move_last_seg_chanel_to_data"):
#        unlabel_transforms.append(MoveSegAsOneHotToData(1, params.get("all_segmentation_labels"), 'seg', 'data'))
#        if params.get("cascade_do_cascade_augmentations") is not None and params.get(
#                "cascade_do_cascade_augmentations"):
#            if params.get("cascade_random_binary_transform_p") > 0:
#                unlabel_transforms.append(ApplyRandomBinaryOperatorTransform(
#                    channel_idx=list(range(-len(params.get("all_segmentation_labels")), 0)),
#                    p_per_sample=params.get("cascade_random_binary_transform_p"),
#                    key="data",
#                    strel_size=params.get("cascade_random_binary_transform_size"),
...
#                                                              output_key='target'))
    ...
    if use_nondetMultiThreadedAugmenter:
        if NonDetMultiThreadedAugmenter is None:
            raise RuntimeError('NonDetMultiThreadedAugmenter is not yet available')
        batchgenerator_unlabel = NonDetMultiThreadedAugmenter(dataloader_un, unlabel_transforms, params.get('num_threads'),
                                                            params.get("num_cached_per_thread"), seeds=seeds_train,
                                                            pin_memory=pin_memory)
    else:
        batchgenerator_unlabel = MultiThreadedAugmenter(dataloader_un, unlabel_transforms, params.get('num_threads'),
                                                      params.get("num_cached_per_thread"),
                                                      seeds=seeds_train, pin_memory=pin_memory)
    # batchgenerator_train = SingleThreadedAugmenter(dataloader_train, tr_transforms)
    # import IPython;IPython.embed()

    return batchgenerator_train, batchgenerator_val, batchgenerator_unlabel
